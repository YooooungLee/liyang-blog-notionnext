    ...
    if (schema[key]?.type && !excludeProperties.includes(schema[key].type)) {
      ...
      switch (schema[key]?.type) {
        case 'date': {
          ...
          // Empty Notion date cells are still present in `value.properties`,
          // but notion-utils returns null for them. Deleting a property from
          // null throws and aborts conversion of the entire database.
          if (dateProperty) {
